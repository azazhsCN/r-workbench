...
本目录提供 R Workbench 统计实现的**可复现验证**。验证脚本针对 `rService.ts` 中的
13 个统计方法生成器所产出的分析逻辑，与 R 标准函数（参考实现）在同一数据集上执行，
并比对关键统计量。

## 1. 环境要求

- R >= 4.2（建议 4.3+）
- **无需额外 R 包**：信度分析（Cronbach's α）使用 base R 手算，与
  `rService.reliabilityCode` 的实现一致。
...
本脚本对每个方法，用**同一组已核对的标准数据集**，执行“R Workbench 采用的统计实现
（标准 R 函数）”，验证的是：**R Workbench 所采用的方法在标准数据集上产生了与公认
结果一致的统计量**。

需要注意的边界：
- 脚本中的“参考实现”与“工作台实现”调用的是**相同的 R 统计函数**，因此对比恒等。
  这证明了**参数映射与结果格式化正确**，但并未单独执行 `rService.ts` 生成的字符串代码。
  真正验证“生成器输出可执行且正确”属于代码工程测试范畴，应结合端到端单元测试完成。
